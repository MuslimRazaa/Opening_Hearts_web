...
function ProductOrderManagment() {
    ...
    const ProductOrderManagmentChart = async (type) => {
        try {
            const response = await apis.ProductOrderManagmentChart(type);
            setCharts(response?.data?.data);
            productOrderManagment(orderType, search, startDate, endDate, 1, page_size);
        } catch (error) {
            setLoading(false)
            Swal.fire({
                icon: 'error',
    ...
    useEffect(() => {
        ProductOrderManagmentChart(type)
    }, [])
                ...
                <div>
                    <div className="row">
                        <div className="col-lg-6">
                            <OrderManagmentBarChart
                                name={charts?.name}
                                value={charts?.bar_chart_order}
                                totalOrder={charts?.total_order}
                                ProductOrderManagmentChart={ProductOrderManagmentChart}
                            />
                        </div>
                        <div className="col-lg-6">
                            <OrderManagmentDoughnutChart
                    ...
                    <div className='orders-table-and-tabs'>
                        ...
                        <div className='table-tabs-main'>
                            <div className="tabs-button-wrapper">
                                <button className={orderType === "" ? "active" : ""} onClick={() => { setOrdertype(""); productOrderManagment(null, search, startDate, endDate  , 1, page_size) }}>All</button>
                                <button className={orderType === "pending" ? "active" : ""} onClick={() => { setOrdertype("pending"); productOrderManagment("pending", search, startDate, endDate , 1, page_size) }}>Pending</button>
                                <button className={orderType === "accepted" ? "active" : ""} onClick={() => { setOrdertype("accepted"); productOrderManagment("accepted", search, startDate, endDate , 1, page_size) }}>Active</button>
                                <button className={orderType === "complete" ? "active" : ""} onClick={() => { setOrdertype("complete"); productOrderManagment("complete", search, startDate, endDate , 1, page_size) }}>Completed</button>
                                <button className={orderType === "refunded" ? "active" : ""} onClick={() => { setOrdertype("refunded"); productOrderManagment("refunded", search, startDate, endDate , 1, page_size) }}>Refunded</button>
                                <button className={orderType === "cancel" ? "active" : ""} onClick={() => { setOrdertype("cancel"); productOrderManagment("cancel", search, startDate, endDate , 1, page_size) }}>Rejected</button>
                            </div>
                            <div className='donation-detail-page-search-bar-table-tab'>
                                <img src={searchIcon} alt="Search icon" />
                                <input type='search' placeholder='Search..' value={search} onChange={(e) => { setSearch(e.target.value); productOrderManagment(orderType, e.target.value, startDate, endDate , 1, page_size) }} />
                            </div>
                                ...
                                <div className="both-side-calendor">
                                    <div className="clander-js">
                                        <div className="calender-dpdown">
                                            ...
                                            <DatePicker
                                                selected={startDate}
                                                onChange={(date) => { setStartDate(formatDate(date)); productOrderManagment(orderType, search, formatDate(date), endDate , 1, page_size) }}
                                    ...
                                    <div className="clander-js">
                                        <div className="calender-dpdown">
                                            ...
                                            <DatePicker
                                                selected={endDate}
                                                onChange={(date) => { setEndDate(formatDate(date)); productOrderManagment(orderType, search, startDate, formatDate(date) , 1, page_size) }}
                                                dateFormat="yyyy-MM-dd"
                                    ...
                                    <div className="page-pagination filter">
                                        ...
                                        {pagination?.total_pages > 12 ?
                                            ...
                                            <PageNumbers
                                                ...
                                                page_size={page_size}
                                                productOrderManagment={productOrderManagment}
...
export default ProductOrderManagment
