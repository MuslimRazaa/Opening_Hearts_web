...
import OrganizationTransactionTable from './OrganizationTransactionTable'
...
const OrganizationWallet = () => {
    ...
    const [formData, setFormData] = useState(
        {
            bank_name: "", account_number: "", holder_name: "", ibm_number: "", routing_number: "", type: "organization"//organization / provider /seller
        }
    )
    ...
    const handleWithdrawSubmit = async (e) => {
        ...
        if (withdrawformData.amount != "" && withdrawformData.bank_id != "" && (withdrawformData.amount <= earning?.amount_balance)) {
            setFullScreenLoader(true);
            const data = new FormData();
            data.append('amount', withdrawformData.amount);
            data.append('bank_id', withdrawformData.bank_id);
            try {
                const response = await apis.createDonationWithdrawRequest(data);
                getOrganizationWalletEarning();
                getOrganizationWalletTransactions(1 , page_size);
    ...
    const getOrganizationWalletEarning = async () => {
        try {
            const response = await apis.getOrganizationWalletEarning();
    ...
    const getOrganizationWalletTransactions = async (page, page_size) => {
        try {
            const response = await apis.getOrganizationWalletTransactions(page, page_size);
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6">
                                <Link to="/product-dashboard/amount-balance-wallet-product" > <div className="wallet-card2">
                                    ...
                                    <p style={{opacity:0}}>Amount Pending</p>
                                </div></Link>
                            </div>
                        </div>
                        <div className="row">
                            <div className="col-md-12">
                                <OrganizationTransactionTable handleOrderNowClick3={handleOrderNowClick3} transactions={transactions} />
                                        ...
                                        <div className="page-pagination filter">
                                            ...
                                            {pagination?.total_pages > 12 ?
                                                ...
                                                <PageNumbers
                                                    ...
                                                    page_size={page_size}
                                                    getOrganizationWalletTransactions={getOrganizationWalletTransactions}
                    ...
                    <div className="wallet-modal-form-wrapper">
                        <form onSubmit={handleWithdrawSubmit}>
                            <div className="wallet-input-ammount">
                                ...
                                {withdrawformData.amount != '' && inputError && withdrawformData.amount > earning?.amount_balance && (<p className="error-input-reg">Withdraw amount can't be greater then {earning?.amount_balance}</p>)}
                            </div>
