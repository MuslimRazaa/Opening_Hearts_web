...
const CustomerDashboardProductDetails = () => {
    ...
    const fetchProductOrderDetails = async () => {
        setIsLoading(true)
        try {
            const response = await productOrderDetail(id)
    ...
    const fetchProductOrderDetailsService = async () => {
        setIsLoading(true)
        try {
    ...
    const handleSuccess = async () => {
        ...
        if (!selectedImages) {
            alert("Please upload Relevent Images.");
            setIsLoading(false)
            return;
        }
    ...
    useEffect(() => {
        fetchProductOrderDetails();
        fetchProductOrderDetailsService();
    }, [])
    ...
    return (
        ...
        <>
            {isLoading ?
                ...
                :
                <>
                    {productOrderDetails?.seller_data?.length > 0 ?
                        <div className='service-detail-wrapper-dashboard-side'>
                            ...
                            <>
                                <div className="row">
                                    {productOrderDetails?.seller_data?.map((seller, index) => (
                                        <div className="order-container">
                                            <div className="order-store-header">
                                                <div className="order-store-profile">
                                                    ...
                                                    <p className="order-store-name">{seller?.shop_name}</p>
                                                </div>
                                                <div className="visit-store">
                                                    <Link to={`/SuplierStore?guid=${seller?.products[0]?.seller?.guid}`}> <p>Visit Store</p></Link>
                                                </div>
                                            </div>
                                            ...
                                            <div className="order-product-list">
                                                {seller?.products?.map((product, index) => (
                                                    <div className="order-card" key={index}>
                                                        <img src={product?.product?.media[0]?.original_url} className="order-product-img" />
                                                        <div className="order-card-content">
                                                            <h5 className="order-product-name">{product?.product?.title}</h5>
                                                            ...
                                                            <p className="order-quantity">
                                                                <span>Quantity:</span> <span className="order-badge">{product.quantity}</span>
                                                            </p>
                                                            <p className="order-price">
                                                                <s className="order-old-price">${product?.product?.price}</s>{" "}
                                                                <span className="order-new-price">${product?.product?.discount_price}</span>
                                                            </p>
                                                            {product.status === "refunded" ? 
                                                            <button className="order-refund-btn" disabled>
                                                                <i className="bi bi-arrow-clockwise"></i> Refund Requested
                                                            </button> : 
                                        ...
                                        <div class="ask-for-return-main">
                                            ...
                                            <div className="upload-section">
                                                <div className="upload-box">
                                                    <img src={upload} alt="" />
                                                    <input type='file' multiple onChange={handleFileChange} style={{ opacity: "0", position: "absolute" }} />
...
export default CustomerDashboardProductDetails
