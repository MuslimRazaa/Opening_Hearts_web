...
const OrderManagmentDoughnutChart = ({value , totalOrder , ProductOrderManagmentChart}) => {
  
const data = {
    labels: [`Pending ${value?.pending}`,`Active ${value?.accepted}`,`Complete ${value?.complete}`, `Refund ${value?.refund}` , `Rejected ${value?.reject}`,],
    datasets: [
      {
        data: [value?.pending,value?.accepted,value?.complete,value?.refund,value?.reject], 
        backgroundColor: ["#ff8369", "#FFB1C1", "#008000","#000", "#ff3535"], 
        hoverBackgroundColor: ["#ff8369", "#FFB1C1", "#008000","#000", "#ff3535"], 
        borderWidth: 0, 
        borderRadius: 4,
      },
    ],
...
export default OrderManagmentDoughnutChart;
